...
p1 = plot(range(1, mult_depth), [error_ciph_by_ciph error_ciph_by_plain error_ciph_by_scalar],
          ...
          xaxis="Number of multiplications", yaxis="L_inf error",
          foreground_color_legend = nothing, background_color_legend = nothing,
          ylims=(0.0, maximum(error_ciph_by_ciph)))
plot!(size=(400,300))
title!("Multiplicative error", titlefontsize=12)
...
p2 = plot(mult_depth_range, [time_ciph_by_ciph time_ciph_by_plain time_ciph_by_scalar],
          ...
          foreground_color_legend = nothing, background_color_legend = nothing,
          ylims=(0.0, maximum(time_ciph_by_ciph)))
plot!(size=(400,300))
title!("Time for a single multiplication", titlefontsize=12)

p3 = plot(p1, p2, layout = @layout([B C]), margin=4mm)
plot!(size=(800,300))
savefig(p3, "out/efficiency/mult.pdf")
