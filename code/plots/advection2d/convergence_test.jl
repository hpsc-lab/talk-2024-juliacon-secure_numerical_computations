...
L2_mean_LW_cfl = []
for N in N_range
    local N_steps = Int(round(timespan[2]*N*maximum(a)/CFL))
    ...
    local _, _, sol_ciph = advection2d("CIPHERTEXT", "Lax-Wendroff", N, N_steps, timespan, a, mult_depth, true, callback)
...
for N in N_range
    ...
    println("Free space: ", Sys.free_memory()/2^20, "MB")
    local callback = StandartCallback(L_2_squared, nothing, nothing, callback_run, frequency)
    local _, _, sol_ciph = advection2d("CIPHERTEXT", "1st order FD", N, N_steps, timespan, a, mult_depth, true, callback)
    #compute ad save errors
