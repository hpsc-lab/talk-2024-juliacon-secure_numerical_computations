...
advection1d("PLAINTEXT", "Lax-Wendroff", N, N_steps, timespan, a, mult_depth, true, callback)
# second run
plain_result = []
callback_record_run(u, t, ode, callback, i) = callback_record(u, t, ode, callback, i, plain_result)
callback = StandartCallback(L_inf, nothing, nothing, callback_record_run, frequency)
timer_plain_conf_vec_LW, _, _ = advection1d("PLAINTEXT", "Lax-Wendroff", N, N_steps, timespan,
...
timer_ciph_conf_vec_LW, _, sol_ciph_LW = advection1d("CIPHERTEXT", "Lax-Wendroff", N, N_steps,
...
callback = StandartCallback(L_inf, nothing, nothing, callback_record_run, frequency)
# first run is not representative in sense of time
advection1d("PLAINTEXT", "1st order FD", N, N_steps, timespan, a, mult_depth, true, callback)
# second run
plain_result = []
callback_record_run(u, t, ode, callback, i) = callback_record(u, t, ode, callback, i, plain_result)
callback = StandartCallback(L_inf, nothing, nothing, callback_record_run, frequency)
timer_plain_conf_vec_FD, _, _ = advection1d("PLAINTEXT", "1st order FD", N, N_steps, timespan,
                                                 a, mult_depth, true, callback)
...
callback_compare_run(u, t, ode, callback, i) = callback_compare(u, t, ode, callback, i, plain_result)
callback = StandartCallback(L_inf, nothing, nothing, callback_compare_run, frequency)
timer_ciph_conf_vec_FD, _, sol_ciph_FD = advection1d("CIPHERTEXT", "1st order FD", N, N_steps,
                                                          timespan, a, mult_depth, true, callback)
...
# get exact solution
sol_exact = advection1d("EXACT", "Lax-Wendroff", N, N_steps, timespan, a, mult_depth, true)
...
CSV.write("out/advection1d/error.csv", table_error)
# time
...
CSV.write("out/advection1d/time.csv", table_time)
# visualization
table_visualization = DataFrame(steps = range(0.0, 1.0, N),
                                sol_ciph_FD = sol_ciph_FD,
                                sol_ciph_LW = sol_ciph_LW,
                                sol_exact = sol_exact)

CSV.write("out/advection1d/visualization.csv", table_visualization)
